...
import { activities, type CreateActivityRequest, type WorkStatus } from "@/lib/api";
import { WORK_STATUS, WORK_STATUS_OPTIONS } from "@/lib/domain";

export function NewActivityDialog({ workPackageId }: { workPackageId: string }) {
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<CreateActivityRequest>({ name: "", status: "not_started" });

  const mutation = useMutation({
    mutationFn: () =>
      activities.create(workPackageId, {
        name: form.name.trim(),
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["activities", workPackageId] });
      toast.success("Үйл ажиллагаа үүслээ");
      setOpen(false);
  ...
  const setText = (k: keyof CreateActivityRequest) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm((f) => ({ ...f, [k]: e.target.value }));

  return (
    <>
      <Button onClick={() => setOpen(true)}>Шинэ үйл ажиллагаа</Button>
      <FormDialog
        open={open}
