...
t(
  ...
  db.locations.length === 162,
  String(db.locations.length),
);
...
// Нэгтгэл
const sum = body<{
  data: { totals: { workItems: number }; groups: { label: string; workItems: number }[] };
...
const bad = handleMock("POST", ["work-items", item.id, "progress"], "", { completedQty: 999999 });
t("Үлдэгдлээс их бол 422", bad.status === 422, String(bad.status));
const neg = handleMock("POST", ["work-items", item.id, "progress"], "", { completedQty: -5 });
t("Сөрөг тоо бол 422", neg.status === 422, String(neg.status));

const before = item.reportedQty;
const okRes = handleMock("POST", ["work-items", item.id, "progress"], "", { completedQty: 10 });
...
for (const gb of ["floor", "workTypeGroup", "contractor", "workType"] as const) {
  const s = body<{
    data: { groups: { label: string; workItems: number; filter: Record<string, unknown> }[] };
...
const designs = body<{ data: { id: string; estimatedItems: number; floors: number }[] }>(
  get("block-designs"),
).data;
...
const noDesign = handleMock("POST", ["projects", "prj-inel-01", "blocks"], "", { name: "В блок" });
...
let job = body<{ data: { id: string; status: string; progress: number; total: number } }>(
  applyRes,
).data;
let guard = 0;
...
const newSum = body<{ data: { totals: { percentage: number; workItems: number } } }>(
  get(`blocks/${newBlock.id}/summary`),
).data;
...
const repSum = body<{
  data: {
    ...
    groups: { label: string; workItems: number; filter: Record<string, unknown> }[];
  };
...
const newUser = post("users", { name: "Б.Болд", email: "bold@cpms.mn", role: "inspector" });
...
for (const [name, no] of [
  ...
  const j = body<{ data: { id: string } }>(
    handleMock("POST", ["block-designs", "dsg-4-svc", "apply"], "", { blockId: nb.id }),
  ).data;
...
handleMock("POST", ["work-items", guarded.id, "progress"], "", { completedQty: 5 });
...
const noChecklist = inspect({ stage: "client", result: "accepted", acceptedQty: 5 });
...
const withFail = inspect({
  ...
  result: "accepted",
  acceptedQty: 5,
  checklist: tmpl!.items.map((i, idx) => ({ itemId: i.id, result: idx === 0 ? "fail" : "pass" })),
});
...
handleMock("POST", ["work-items", guarded.id, "progress"], "", { completedQty: 3 });
...
const free = getDb().workItems.find(
  (w) => !["Угсралт", "Өрлөг", "Засал"].includes(w.workType.groupName) && w.remainingQty > 20,
)!;
...
handleMock("POST", ["work-items", free.id, "progress"], "", { completedQty: 4 });
...
t(
  ...
  handleMock("POST", ["work-items", zeroItem.id, "progress"], "", { completedQty: 1 }).status ===
    422,
);
...
t(
  ...
  handleMock("POST", ["work-items", zeroItem.id, "progress"], "", { completedQty: 10 }).status ===
    201,
);
...
const freshJob = body<{ data: { id: string } }>(
  handleMock("POST", ["block-designs", "dsg-4-svc", "apply"], "", { blockId: freshBlock.id }),
).data;
...
const workedItem = getDb().workItems.find(
  (w) => w.blockId === freshBlock.id && w.contractor?.id === goo.id && w.remainingQty > 5,
)!;
handleMock("POST", ["work-items", workedItem.id, "progress"], "", { completedQty: 2 });
...
t("Оноосны дараа гүйцэтгэгч ажлаа харна", repSees === target.unassigned, String(repSees));
...
// байх ёстой.
const emptyBlock = body<{ data: { id: string; floors: number; unitCount: number } }>(
  handleMock("POST", ["projects", "prj-inel-01", "blocks"], "", {
    name: "Хоосон блок",
    buildingNo: "77",
    floors: 5,
...
t(
  ...
  handleMock("POST", ["projects", "prj-inel-01", "blocks"], "", { name: "Мэдээлэлгүй" }).status ===
    422,
);
...
const resched = handleMock("POST", ["blocks", emptyBlock.id, "schedule"], "", { taktDays: 12 });
...
const badTakt = handleMock("POST", ["blocks", emptyBlock.id, "schedule"], "", { taktDays: 0 });
...
t(
  ...
  handleMock("POST", ["work-items", addedItem.id, "progress"], "", { completedQty: 5 }).status ===
    201,
);
...
// Мөн тусдаа «дахин хийх» мөр үүсгэвэл блокийн төлөвлөгөө хоёр дахин болно.
{
  ...
  handleMock("POST", ["work-items", target.id, "progress"], "", { completedQty: planned });
